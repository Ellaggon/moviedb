...
export default function SearchButton({ clicked, handleClick }: { clicked: boolean; handleClick: () => void }) {
  ...
  return (
    <div className={`transition-all ${
      clicked ? "max-w-md border-2 border-white rounded-lg ml-auto" : ""
      } ${
      ...
      <form onSubmit={onSubmit} className={`flex items-center gap-2 transition-all `}>
        <input 
          ...
          onChange={onChange} 
          placeholder="Search" 
          className={`bg-transparent outline-none text-white transition-all ${
            clicked ? "w-full px-3 py-1 opacity-100" : "w-0 opacity-0"
            } ${
              error ? "border-red-500" : ""
              }`} />
